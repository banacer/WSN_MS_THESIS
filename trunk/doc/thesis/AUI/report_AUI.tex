...
\paragraph{}
Another migration is taking place in the power grid that is evolving towards the \gls{sg}. In fact, Information Technology (IT) is playing a key role in implementing needed functions such as \gls{dr}\glsreset{dr} and integrating Distributed and Renewable Energy Sources into \gls{sg}. The \gls{iot} is expected to enable a smooth integration of Distributed Renewable Energies within the electric grid.
\paragraph{}
This thesis focuses on the development of an \gls{iot} test-bed, using wireless sensors, that will be used for Smart Grid applications. The test-bed provides an IPv6 gateway, remote control of wireless sensor nodes which is built on top of \gls{6lowpan}\glsreset{6lowpan}, and mobile access to the system.
\paragraph{}
